...
public class RocketJumperItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand p_41192_) {
        ...
        if (!level.isClientSide) {
            RocketJumperEntity rocketJumperEntity = new RocketJumperEntity(level, player.getX(), player.getEyeY(), player.getZ(), player);
            //rocketJumperEntity.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
            level.addFreshEntity(rocketJumperEntity);
        }
